$(function() {
    ...
    if(cssNumber == 2){
        ...
        $('#headerLogo').attr('src','img1/KAKEIBO2.svg');
        $('#calactar').attr('src','img1/ヤドカリ.svg'); 
        $('#noEitData').attr('src','img1/ヤドカリ3.svg');
    ...
    let addHL = function(type,newname,firstmoney,firstmoney2,i) {
        ...
        if(cssNumber == 2) {
            if(type === '現金') {
                acount = acount.replace('A', 'img1/CASH2.svg');
            } else if(type === '銀行') {
                acount = acount.replace('A', 'img1/BANK2.svg');
            } else if(type === 'クレジット') {
                acount = acount.replace('A', 'img1/CREDIT2.svg');
            } else if(type === '電子マネー') {
                acount = acount.replace('A', 'img1/PAY2.svg');
            } else if(type === 'その他') {
                acount = acount.replace('A', 'img1/OTHER2.svg');
            }
    
        }else {
            if(type === '現金') {
                acount = acount.replace('A', 'img1/CASH.svg');
            } else if(type === '銀行') {
                acount = acount.replace('A', 'img1/BANK.svg');
            } else if(type === 'クレジット') {
                acount = acount.replace('A', 'img1/CARD.svg');
            } else if(type === '電子マネー') {
                acount = acount.replace('A', 'img1/Pay.svg');
            } else if(type === 'その他') {
                acount = acount.replace('A', 'img1/OTHER.svg');
            }
    ...
    $('.remove').click(function() {
        let result = window.confirm('このアカウントに関する全てのデータが削除されます。\n本当に削除しますか？');
        if(result) {
            let i = $(this).val();
            removeHLData(i);
            location.reload();
...
　　makeEitArr();
    ...
    let readEitList = function(i) {
        ...
        if(eitx.length == 0) {
        document.getElementById('noEitData').style.display = 'block';
        }else{
...
　　makeLbArr();
    ...
    // ヘッダーの高さに合わせてアカウントリストを下げる
    $(function() {
        let height = $("#header").height();
        $(".acountPlate").css("margin-top", height);
      });
    
    // 貸し借りリストが無いとき、アカウントリストの下にスペースができるようにする（スクロールしきれるように）
    let lbList = document.getElementById('lbList');
    if(lbList.children.length == 0) {
        $('#homeList').append('<div id="invisible"></div>');
    }
